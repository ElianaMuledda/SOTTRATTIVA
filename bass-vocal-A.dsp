...
Ebassf3 = fi.highpass(order, fcut): fi.lowpass(order, fcut): *(gain) : meter 
  with{
    ...
    gain = f3group(vslider("[01] GAIN", 14.2, -96, +24, 0.1)) : ba.db2linear : si.smoo;
...
Ebassf4 = fi.highpass(order, fcut): fi.lowpass(order, fcut): *(gain) : meter 
  with{
    ...
    gain = f4group(vslider("[01] GAIN", -27.6, -96, +24, 0.1)) : ba.db2linear : si.smoo;
...
Ebassf5 = fi.highpass(order, fcut): fi.lowpass(order, fcut): *(gain) : meter 
  with{
    ...
    gain = f5group(vslider("[01] GAIN", -33.4, -96, +6, 0.1)) : ba.db2linear : si.smoo;
