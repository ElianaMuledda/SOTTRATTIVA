...
Ebassf3 = fi.highpass(order, fcut): fi.lowpass(order, fcut): *(gain) : meter 
  with{
    ...
    gain = f3group(vslider("[01] GAIN", 13.6, -96, +24, 0.1)) : ba.db2linear : si.smoo;
...
Ebassf4 = fi.highpass(order, fcut): fi.lowpass(order, fcut): *(gain) : meter 
  with{
    ...
    gain = f4group(vslider("[01] GAIN", 0.9, -96, +24, 0.1)) : ba.db2linear : si.smoo;
...
Ebassf5 = fi.highpass(order, fcut): fi.lowpass(order, fcut): *(gain) : meter 
  with{
    ...
    gain = f5group(vslider("[01] GAIN", -15.7, -96, +6, 0.1)) : ba.db2linear : si.smoo;
