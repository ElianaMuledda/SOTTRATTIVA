...
Ebassf2 = fi.highpass(order, fcut): fi.lowpass(order, fcut) : *(gain) : meter 
  with{
    ...
    gain = f2group(vslider("[01] GAIN", 12.3, -96, +12, 0.1)) : ba.db2linear : si.smoo;
...
Ebassf3 = fi.highpass(order, fcut): fi.lowpass(order, fcut): *(gain) : meter 
  with{
    ...
    gain = f3group(vslider("[01] GAIN", -6.7, -96, +6, 0.1)) : ba.db2linear : si.smoo;
...
Ebassf5 = fi.highpass(order, fcut): fi.lowpass(order, fcut): *(gain) : meter 
  with{
    ...
    gain = f5group(vslider("[01] GAIN", -12.8, -96, +6, 0.1)) : ba.db2linear : si.smoo;
...
process =  no.noise <: hgroup("U FORMANTICO", Ebassf1, Ebassf2, Ebassf3, Ebassf4, Ebassf5) :>outmeter ;

 // vocale U voce di basso
 // 350 600 2400 2675 2950 (frequenza)
 // 40 80 100 120 120 (larghezza di banda)
 // 0 -20 -32 -28 -36 (decibel)
