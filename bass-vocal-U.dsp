...
Ebassf3 = fi.highpass(order, fcut): fi.lowpass(order, fcut): *(gain) : meter 
  with{
    ...
    gain = f3group(vslider("[01] GAIN", -11.7, -96, +6, 0.1)) : ba.db2linear : si.smoo;
...
Ebassf4 = fi.highpass(order, fcut): fi.lowpass(order, fcut): *(gain) : meter 
  with{
    ...
    gain = f4group(vslider("[01] GAIN", -11, -96, +6, 0.1)) : ba.db2linear : si.smoo;
...
Ebassf5 = fi.highpass(order, fcut): fi.lowpass(order, fcut): *(gain) : meter 
  with{
    ...
    gain = f5group(vslider("[01] GAIN", -17.8, -96, +6, 0.1)) : ba.db2linear : si.smoo;
