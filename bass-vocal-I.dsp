...
Ebassf1 = fi.highpass(order, fcut) : fi.lowpass(order, fcut) : *(gain) : meter 
  with{
    ...
    gain = f1group(vslider("[01] GAIN", -19.2, -96, +6, 0.1)) : ba.db2linear : si.smoo;
...
Ebassf2 = fi.highpass(order, fcut): fi.lowpass(order, fcut) : *(gain) : meter 
  with{
    ...
    gain = f2group(vslider("[01] GAIN", -0.9, -96, +6, 0.1)) : ba.db2linear : si.smoo;
...
Ebassf5 = fi.highpass(order, fcut): fi.lowpass(order, fcut): *(gain) : meter 
  with{
    ...
    gain = f5group(vslider("[01] GAIN", -2.2, -96, +6, 0.1)) : ba.db2linear : si.smoo;
